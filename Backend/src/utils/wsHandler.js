            ...
            try {
                ...
                if (data.role === 'driver') {
                    if (data.action === 'register') {
                        ...
                        const vehicleRes = await pool.query("SELECT vehicle FROM vehicles WHERE driver_id = $1", [ws.driver_id]);
                        ws.vehicle = vehicleRes.rows[0]?.vehicle;

                        ws.cur_location = data.cur_location;
                        drivers.set(ws.driver_id, { ws, cur_location: ws.cur_location, vehicle: ws.vehicle, available: true });
                        ...
                        await pool.query("UPDATE ride_req SET arrived_time = CURRENT_TIMESTAMP, status = 'completed' WHERE ride_id = $1", [data.ride_id]);
                        await pool.query("UPDATE rides SET start_time = CURRENT_TIMESTAMP WHERE ride_id = $1", [data.ride_id]);


                        notifyRiders({ status: 'Arrived', driver_id: ws.driver_id, ride_id: data.ride_id } );
                    }
                        ...
                        const result = await pool.query("SELECT fare FROM rides WHERE ride_id = $1", [data.ride_id]);
                        const fare = result.rows[0]?.fare || 0;

                        await pool.query("UPDATE rides SET  end_time =CURRENT_TIMESTAMP, status = 'completed' WHERE ride_id = $1", [data.ride_id]);

                        notifyRiders({ status: 'Trip ended',  ride_id: data.ride_id, driver_id: ws.driver_id, Trip_Fare : fare} );
                    }
                }
                // rider actions
                    ...
                    // }
                     if (data.action === 'ride_request') {
                        ...
                        ws.send(JSON.stringify({message: 'Rider registered successfully',  fares }));
                    }
                        ...
                        try {
                            notifyDriversSequentially(
                                ...
                                async (driverWs) => {
                                    ...
                                    await pool.query("UPDATE ride_req SET driver_id = $1, status = 'booked', res_time = CURRENT_TIMESTAMP WHERE rider_id = $2 AND status = 'available'",
                                    ...
                                    await pool.query(
                                        ...
                                        [ws.ride_id, ws.rider_id, data.origin, data.destination, data.fare, data.vehicle]
                                    );
                            ...
                            return;
                                    ...
                                    const { driver_name, driver_phone, model, license_plate, color } = result.rows[0];

                                    ws.send(JSON.stringify({ status: 'accepted  ', ride_id : ws.ride_id ,driver_id: driverWs.driver_id, driver_name: driver_name, driver_phone: driver_phone, model: model, license_plate: license_plate, color: color }));
                                ...
                                async () => {
                                    ...
                                    await pool.query("UPDATE ride_req SET  status = 'Failed' WHERE rider_id = $1 AND status = 'available'" ,[ ws.rider_id])
